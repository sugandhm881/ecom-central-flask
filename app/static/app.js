...
function renderAdsetPerformanceDashboard() {
    ...
    adsetPerformanceData.forEach(adset => {
        ...
        // Updated RTO% calculation including Cancelled:
        ...
        let adsetRow = `
          <tr class="border-b border-slate-200 bg-slate-50 cursor-pointer" data-adset-id="${adset.id}">
            ...
            <td class="py-3 px-4 text-right font-bold">${formatCurrency(spend)}</td>               <!-- Spend -->
            <td class="py-3 px-4 text-right font-bold">${formatNumber(t)}</td>                      <!-- Total Orders -->
            <td class="py-3 px-4 text-right font-bold text-green-600">${formatNumber(adset.deliveredOrders)}</td>
            <td class="py-3 px-4 text-right font-bold">${formatCurrency(adset.deliveredRevenue)}</td> <!-- Delivered Revenue -->
            ...
            <td class="py-3 px-4 text-right font-bold text-red-600">${formatPercent(o)}</td>           <!-- RTO% -->
            <td class="py-3 px-4 text-right font-bold">${formatCurrency(costPerOrder)}</td>          <!-- CPO (fixed) -->
            <td class="py-3 px-4 text-right font-bold">${r.toFixed(2)}x</td>                         <!-- ROAS -->
          </tr>`;

        (adset.terms || []).forEach(term => {
            const tTerm = term.totalOrders || 0;

            // Updated term-level RTO% including Cancelled:
            ...
            adsetRow += `
              <tr class="adset-term-row hidden border-b border-slate-100" data-parent-adset-id="${adset.id}">
                ...
                <td class="py-2 px-4 text-right text-sm">${formatCurrency(spendTerm)}</td>                 <!-- Spend -->
                <td class="py-2 px-4 text-right text-sm">${formatNumber(tTerm)}</td>                        <!-- Total Orders -->
                <td class="py-2 px-4 text-right text-sm text-green-600">${formatNumber(term.deliveredOrders)}</td>
                <td class="py-2 px-4 text-right text-sm">${formatCurrency(term.deliveredRevenue)}</td>       <!-- Delivered Revenue -->
                ...
                <td class="py-2 px-4 text-right text-sm text-red-600">${formatPercent(oTerm)}</td>             <!-- RTO% -->
                <td class="py-2 px-4 text-right text-sm">${formatCurrency(costPerOrderTerm)}</td>            <!-- CPO (fixed) -->
                <td class="py-2 px-4 text-right text-sm">${rTerm.toFixed(2)}x</td>                            <!-- ROAS -->
              </tr>`;
        });
...
// DOMContentLoaded event listener and initialization code remain unchanged  
...
o.forEach(order=>{  
    // FIX: Use buyerName if name is N/A  
...
async function handleExcelDownload() {  
    ...
    const endpoint = `/download-excel-report?since=${since}&until=${until}`;  
    try {  
        const blob = await fetchApiData(endpoint, "Failed to generate Excel report");  
...
document.addEventListener('DOMContentLoaded', () => {  
    ...
    downloadPdfBtn = document.getElementById('download-pdf-btn') || document.getElementById('download-adset-pdf');  
    downloadExcelBtn = document.getElementById('download-excel-btn') || document.getElementById('download-adset-excel');  
    ...
    navOrdersDashboard?.addEventListener('click', () => navigate('orders-dashboard'));  
    navOrderInsights?.addEventListener('click', () => navigate('order-insights'));  
    navAdPerformance?.addEventListener('click', () => navigate('ad-performance'));  
    navAdsetBreakdown?.addEventListener('click', () => navigate('adset-breakdown'));  
