...
async function fetchApiData(endpoint, errorMessage, options = {}) {
    ...
    try {
        ...
        const isExcel = contentType && contentType.includes('spreadsheetml.sheet'); // Check for Excel
        if (contentType && contentType.includes('application/pdf') || isExcel) { // <-- Checks for PDF OR Excel
...
const fetchBuyerInfoForModal = (orderId) => fetchApiData(`/get-amazon-buyer-info?orderId=${orderId}`, 'Failed to fetch buyer name.');
...
async function downloadShipmentLabel(awb) {
    ...
    } catch (err) { /* Error is already handled by fetchApiData */ }
}
...
// --- ADD THE NEW INVOICE DOWNLOAD FUNCTION ---
...
function renderOrderDetails(order) {
    ...
    if (order.platform === 'Shopify') {
        ...
        // If the order has an AWB, show download buttons instead of the create button
        if (order.awb) {
            primaryActionsHtml += `
...
async function handleAdsetDateChange() {
    ...
    if (startDate && endDate) {
        ...
        // --- THIS IS THE FIX ---
        // Get the selected filter type from the new dropdown
        const dateFilterType = adsetDateFilterTypeEl ? adsetDateFilterTypeEl.value : 'order_date';
        
        // Pass the new filter type to the API
...
function renderAdPerformancePage(){renderDailyPerformance();renderAdPerformanceCharts();}
function renderDailyPerformance(){const totals=performanceData.reduce((a,d)=>(a.spend+=d.spend,a.revenue+=d.revenue,a.orders+=d.totalOrders,a.delivered+=d.deliveredOrders,a.rto+=d.rtoOrders,a.cancelled+=d.cancelledOrders,a),{spend:0,revenue:0,orders:0,delivered:0,rto:0,cancelled:0});const roas=totals.spend>0?totals.revenue/totals.spend:0;const renderKpi=(e,t,v)=>{e.innerHTML=`<p class="text-sm font-medium text-slate-500">${t}</p><p class="text-3xl font-bold text-slate-800 mt-2">${v}</p>`};renderKpi(adKpiElements.totalSpend,'Total Spend',formatCurrency(totals.spend));renderKpi(adKpiElements.totalRevenue,'Total Revenue',formatCurrency(totals.revenue));renderKpi(adKpiElements.roas,'ROAS',`${roas.toFixed(2)}x`);renderKpi(adKpiElements.delivered,'Delivered',formatNumber(totals.delivered));renderKpi(adKpiElements.rto,'RTO',formatNumber(totals.rto));renderKpi(adKpiElements.cancelled,'Cancelled',formatNumber(totals.cancelled));performanceTableBody.innerHTML='';[...performanceData].reverse().forEach(d=>{const c=d.totalOrders>0?d.spend/d.totalOrders:0;const r=d.spend>0?d.revenue/d.spend:0;const t=d.totalOrders>0?d.rtoOrders/d.totalOrders:0;performanceTableBody.innerHTML+=`<tr class="border-b border-slate-100"><td class="py-3 px-4 font-medium">${new Date(d.date).toLocaleDateString('en-GB',{day:'short',month:'short',timeZone:'UTC'})}</td><td class="py-3 px-4 text-right">${formatCurrency(d.spend)}</td><td class="py-3 px-4 text-right">${formatNumber(d.totalOrders)}</td><td class="py-3 px-4 text-right">${formatCurrency(d.revenue)}</td><td class="py-3 px-4 text-right">${formatCurrency(c)}</td><td class="py-3 px-4 text-right font-semibold">${r.toFixed(2)}x</td><td class="py-3 px-4 text-right text-green-600">${formatNumber(d.deliveredOrders)}</td><td class="py-3 px-4 text-right text-red-600">${formatNumber(d.rtoOrders)}</td><td class="py-3 px-4 text-right text-gray-600">${formatNumber(d.cancelledOrders||0)}</td><td class="py-3 px-4 text-right text-yellow-600">${formatNumber(d.inTransitOrders||0)}</td><td class="py-3 px-4 text-right text-blue-600">${formatNumber(d.processingOrders||0)}</td><td class="py-3 px-4 text-right text-red-600 font-medium">${formatPercent(t)}</td></tr>`})}
function renderAdPerformanceCharts(){if(spendRevenueChartInstance)spendRevenueChartInstance.destroy();if(orderStatusChartInstance)orderStatusChartInstance.destroy();const labels=performanceData.map(d=>new Date(d.date).toLocaleDateString('en-GB',{day:'short',month:'short',timeZone:'UTC'}));spendRevenueChartInstance=new Chart(spendRevenueChartCanvas,{type:'line',data:{labels,datasets:[{label:'Ad Spend',data:performanceData.map(d=>d.spend),borderColor:'#ef4444',backgroundColor:'#fee2e2',fill:true,yAxisID:'y'},{label:'Revenue',data:performanceData.map(d=>d.revenue),borderColor:'#22c55e',backgroundColor:'#dcfce7',fill:true,yAxisID:'y'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{title:{display:true,text:'Spend vs. Revenue'}},scales:{y:{beginAtZero:true}}}});const totals=performanceData.reduce((a,d)=>(a.delivered+=d.deliveredOrders,a.rto+=d.rtoOrders,a.cancelled+=d.cancelledOrders,a),{delivered:0,rto:0,cancelled:0});orderStatusChartInstance=new Chart(orderStatusChartCanvas,{type:'doughnut',data:{labels:['Delivered','RTO','Cancelled'],datasets:[{data:[totals.delivered,totals.rto,totals.cancelled],backgroundColor:['#22c55e','#ef4444','#64748b']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{title:{display:true,text:'Order Status Breakdown'}}}})}

function renderAdsetPerformanceDashboard(){adsetPerformanceTableBody.innerHTML='';if(!adsetPerformanceData||adsetPerformanceData.length===0){adsetPerformanceTableBody.innerHTML=`<tr><td colspan="12" class="p-4 text-center text-slate-500">No ad set data found for this period.</td></tr>`;return}
adsetPerformanceData.forEach(adset=>{const t=adset.totalOrders;const c=t>0?adset.spend/t:0;const r=adset.spend>0?adset.revenue/adset.spend:0;const o=t>0?adset.rtoOrders/t:0;let adsetRow=`<tr class="border-b border-slate-200 bg-slate-50 cursor-pointer" data-adset-id="${adset.id}"><td class="py-3 px-4 font-bold text-sm text-slate-800 text-left">${adset.name}</td><td class="py-3 px-4 text-sm text-slate-500 text-left">${(adset.terms||[]).length} terms</td><td class="py-3 px-4 text-right font-bold">${formatCurrency(adset.spend)}</td><td class="py-3 px-4 text-right font-bold">${formatNumber(t)}</td><td class="py-3 px-4 text-right font-bold text-green-600">${formatNumber(adset.deliveredOrders)}</td><td class="py-3 px-4 text-right font-bold text-red-600">${formatNumber(adset.rtoOrders)}</td><td class="py-3 px-4 text-right font-bold text-slate-500">${formatNumber(adset.cancelledOrders)}</td><td class="py-3 px-4 text-right font-bold text-blue-600">${formatNumber(adset.inTransitOrders||0)}</td><td class="py-3 px-4 text-right font-bold text-yellow-600">${formatNumber(adset.processingOrders||0)}</td><td class="py-3 px-4 text-right font-bold text-red-600">${formatPercent(o)}</td><td class="py-3 px-4 text-right font-bold">${formatCurrency(c)}</td><td class="py-3 px-4 text-right font-bold">${r.toFixed(2)}x</td></tr>`;(adset.terms||[]).forEach(term=>{const t=term.totalOrders;const c=t>0?term.spend/t:0;const r=term.spend>0?term.revenue/term.spend:0;const o=t>0?term.rtoOrders/t:0;adsetRow+=`<tr class="adset-term-row hidden border-b border-slate-100" data-parent-adset-id="${adset.id}"><td class="py-2 px-8 text-sm text-slate-600 text-left" colspan="2">${term.name||term.id}</td><td class="py-2 px-4 text-right text-sm">${formatCurrency(term.spend)}</td><td class="py-2 px-4 text-right text-sm">${formatNumber(t)}</td><td class="py-2 px-4 text-right text-sm text-green-600">${formatNumber(term.deliveredOrders)}</td><td class="py-2 px-4 text-right text-sm text-red-600">${formatNumber(term.rtoOrders)}</td><td class="py-2 px-4 text-right text-sm text-slate-500">${formatNumber(term.cancelledOrders)}</td><td class="py-2 px-4 text-right text-sm text-blue-600">${formatNumber(term.inTransitOrders||0)}</td><td class="py-2 px-4 text-right text-sm text-yellow-600">${formatNumber(term.processingOrders||0)}</td><td class="py-2 px-4 text-right text-sm text-red-600">${formatPercent(o)}</td><td class="py-2 px-4 text-right text-sm">${formatCurrency(c)}</td><td class="py-2 px-4 text-right text-sm">${r.toFixed(2)}x</td></tr>`});adsetPerformanceTableBody.innerHTML+=adsetRow});adsetPerformanceTableBody.querySelectorAll('tr[data-adset-id]').forEach(row=>{row.addEventListener('click',()=>{const adsetId=row.dataset.adsetId;document.querySelectorAll(`tr[data-parent-adset-id="${adsetId}"]`).forEach(termRow=>{termRow.classList.toggle('hidden')})})})}
...
o.forEach(order=>{const r=document.createElement('tr');r.className=`order-row border-b border-slate-100 cursor-pointer`;r.dataset.orderId=order.id;r.innerHTML=`<td class="p-4"><img src="${platformLogos[order.platform]||''}" class="w-6 h-6" alt="${order.platform}"></td><td class="p-4 text-slate-600 text-sm">${order.date}</td><td class="p-4 font-semibold text-slate-700">${order.id}</td><td class="p-4 font-medium">${order.name}</td><td class="p-4">${formatCurrency(order.total)}</td><td class="p-4"><span class="px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(order.status)}">${order.status}</span></td>`;r.addEventListener('click',()=>openOrderModal(order.id));ordersListEl.appendChild(r)})}
function openOrderModal(o){const order=allOrders.find(t=>t.id===o);if(order){selectedOrderId=order.id;renderOrderDetails(order);orderModal.classList.remove('modal-hidden');orderModal.classList.add('modal-visible');if(order.platform==='Amazon'&&order.name==='N/A'){fetchBuyerInfoForModal(order.id).then(data=>{const i=allOrders.findIndex(t=>t.id===o);if(i!==-1&&data){allOrders[i].name=data.name||"N/A";renderOrderDetails(allOrders[i]);renderAllDashboard()}}).catch(()=>{})}}}
...
async function handleExcelDownload() {
    ...
    const url = `/download-excel-report?since=${since}&until=${until}`;
    try {
        const blob = await fetchApiData(url, "Failed to generate Excel report");
