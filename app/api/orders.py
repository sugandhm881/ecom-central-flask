...
def normalize_shopify_order(order):
    ...
    tags_string = str(tags_raw).lower() if tags_raw else ''
    
    # Logic: RapidShyp eligible UNLESS tag contains 'docpharma: in-progress'
    is_rapidshyp_eligible = 'docpharma: in-progress' not in tags_string
    ...
    return {
        ...
        "awb": awb,
        "isRapidShyp": is_rapidshyp_eligible,
        "locationId": str(order.get('location_id') or 'N/A'), 
        "tags": tags_raw 
    }
...
def get_orders():
    ...
    try:
        # UPDATED: Renamed variable and ensured it is strictly 30 days
        thirty_days_ago = (datetime.now() - timedelta(days=30)).isoformat()
        
        shopify_params = {
            'status': 'any', 
            'limit': 250,
            'created_at_min': thirty_days_ago, # Uses the 30-day filter
        ...
        print("\n[Orders Endpoint] Fetching all paginated Shopify orders (Last 30 Days)...")
